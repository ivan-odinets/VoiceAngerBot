...
Message::Message(const QJsonObject& message)
{
    ...
    x audio               Audio     Optional. Message is an audio file, information about the file
      ...
      video_note          VideoNote	Optional. Message is a video message, information about the file
    ...
    */
    ...
    if (message.contains("photo")) {
        ...
        string = message.value("caption").toString();
    ...
    if (message.contains("video_note")) {
        obj = message.value("video_note").toObject();
        videoNote = VideoNote(obj);
        type = Message::VideoNoteType;
    }
